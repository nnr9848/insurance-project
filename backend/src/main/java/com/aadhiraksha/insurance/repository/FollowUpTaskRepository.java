...
public interface FollowUpTaskRepository extends JpaRepository<FollowUpTask, Long> {
    ...
    @Query("SELECT f FROM FollowUpTask f WHERE f.advisor.manager.id = :managerId AND f.scheduledDatetime BETWEEN :start AND :end AND f.status = 'PENDING' ORDER BY f.scheduledDatetime ASC")
    List<FollowUpTask> findDueTodayForManager(@Param("managerId") Long managerId, @Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    @Query("SELECT f FROM FollowUpTask f WHERE f.scheduledDatetime BETWEEN :start AND :end AND f.status = 'PENDING' ORDER BY f.scheduledDatetime ASC")
    List<FollowUpTask> findAllDueToday(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
    ...
    @Query("SELECT f FROM FollowUpTask f WHERE f.scheduledDatetime < :now AND f.status = 'PENDING' ORDER BY f.scheduledDatetime ASC")
    List<FollowUpTask> findAllOverdue(@Param("now") LocalDateTime now);
